...
class CalendarPicker extends JDialog {
    ...
    private void buildUI() {
        ...
        monthLbl.setFont(UITheme.FONT_HEADER);
        ...
        todayBtn.setPreferredSize(new Dimension(90, 28));
        clearBtn.setPreferredSize(new Dimension(80, 28));

        JPanel bot = new JPanel(new FlowLayout(FlowLayout.CENTER, 6, 2));
        bot.setOpaque(false);
        bot.add(todayBtn);
        bot.add(clearBtn);
    ...
    private void rebuildCal(JLabel monthLbl) {
        ...
        for (String d : new String[]{"Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"}) {
            ...
            l.setForeground(UITheme.ACCENT_TEAL);
            calPanel.add(l);
        }
        ...
        for (int day = 1; day <= viewing.lengthOfMonth(); day++) {
            ...
            btn.setForeground(isSel ? Color.WHITE : UITheme.TEXT_PRIMARY);
            btn.setBackground(isSel ? UITheme.ACCENT_BLUE : UITheme.BG_TABLE);
            btn.setOpaque(true);
            btn.setBorderPainted(false);
            btn.setFocusPainted(false);
            btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
